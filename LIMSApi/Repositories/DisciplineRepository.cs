...
namespace LIMSApi.Repositories
{
    public class DisciplineRepository : IDisciplineRepository
    {
        ...
        public async Task<PagedResponse<object>> GetAllDisciplines(PageFilter filter)
        {
            ...
            {
                foreach (var filterDiscipline in filter.Filters)
                {
                    if (string.IsNullOrWhiteSpace(filterDiscipline.Value))
                    {
                        continue;
                    }
                    var propertyName = filterDiscipline.Key;
                    var value = filterDiscipline.Value;
