    ...
    public class TPIMasterRepository : ITPIMasterRepository
    {
        ...
        public async Task<PagedResponse<object>> GetAllTPIs(PageFilter filter)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filter.searchTerm))
            {
                ...
                _query = _query.Where(x =>
                    (x.AgencyName != null && x.AgencyName.ToLower().Contains(search))
                    || x.EmailId.ToLower().Contains(search)
                    || x.ContactNo.ToLower().Contains(search)
                );
            }
        ...
        public async Task<List<DropdwonSelector>> GetTPIDropdown(string? searchTerm, int pageNo = 0, int pageSize = 20)
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                ...
                _query = _query.Where(x => x.ID.ToString().Contains(search) || (x.AgencyName != null && x.AgencyName.ToLower().Contains(search)));
            }
