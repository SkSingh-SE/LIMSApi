...
namespace LIMSApi.Repositories
{
    public class TestTypeRepository : ITestTypeRepository
    {
        ...
        {
            return await _context.TestTypeMasters.FirstOrDefaultAsync(x => x.ID == id && x.IsActive);
        }
        ...
        {
            var _query = from c in _context.TestTypeMasters where c.IsActive select c;
        ...
        public async Task<List<DropdwonSelector>> GetTestTypeDropdown(string? searchTerm, int pageNo = 0, int pageSize = 20)
        {
            ...
            var _query = from a in _context.TestTypeMasters where a.IsActive select a;
        ...
        {
            return await _context.TestTypeMasters.AnyAsync(x => x.Name == name && x.IsActive);
        }

        public async Task<bool> ExistsByNameAndNotId(string name, long Id)
        {
            return await _context.TestTypeMasters.AnyAsync(x => x.Name == name && x.ID != Id && x.IsActive);
        }
    }
}
