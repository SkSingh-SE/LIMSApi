...
namespace LIMSApi.Repositories
{
    public class SubContractorRepository : ISubContractorRepository
    {
        ...
        public async Task<PagedResponse<object>> GetAllSubContractors(PageFilter filter)
        {
            ...
            {
                foreach (var filterSubContractor in filter.Filters)
                {
                    if (string.IsNullOrWhiteSpace(filterSubContractor.Value))
                    {
                        continue;
                    }
                    var propertyName = filterSubContractor.Key;
                    var value = filterSubContractor.Value;
